...
ROBOT_SPEED = 500     # mm/s(clock's pic)
...
class Robot:
    ...
    def calculateMove(self, method):
        if method == 0:
            ...
            if len(self.path) > 0:
                distance_to_end_point = int(abs((self.position_x - self.path[0].x) + (self.position_y - self.path[0].y)))
                if distance_to_end_point < ROBOT_SPEED and (self.position_x - self.path[0].x == 0 or self.position_y - self.path[0].y == 0):
                    return self.path[0]
                else:
                    x_diff = self.position_x - self.path[0].x
                    if x_diff != 0:
                        if self.path[0].x - self.position_x < 0:
                            new_pos = Point(int(self.position_x - ROBOT_SPEED), int(self.position_y))
                        else:
                            new_pos = Point(int(self.position_x + ROBOT_SPEED), int(self.position_y))
                    else:
                        if self.path[0].y - self.position_y < 0:
